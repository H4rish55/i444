...
export class Errs {
  private errs: Err[] = [];
  add(err: Err) { this.errs.push(err); return this; }
  toString() {
    return this.errs.map(e => e.toString()).join('\n');
  }
  errors() { return [ ... this.errs ]; }
  nErrors() { return this.errs.length; }
  at(index: number) : Err { return this.errs[index]; }
  static fromErr(e: Err) {
    return new Errs().add(e);
  }
  static err(e: string|Error|Object, code: string = DEFAULT_ERR_CODE,
	     opts: ErrOptions = {}) {
    return Errs.fromErr(Err.err(e, code, opts));
  }  
}
